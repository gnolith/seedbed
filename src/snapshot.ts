...
export async function createInstallationSnapshot(
  ...
  return adapter.withMaintenanceLock(async () => {
    ...
    try {
      const databaseBytes = await readFile(temporaryDatabase);
      const identity = await readSnapshotIdentity(temporaryDatabase);
      const blobs = await readBlobEntries(adapter.blobPath);
      ...
      const compressed = await gzipAsync(Buffer.from(JSON.stringify(envelope)), { level: 9 });
      try {
        await writeFile(destination, compressed, { flag: 'wx', mode: 0o600 });
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code === 'EEXIST') {
          throw snapshotError(`Snapshot already exists at ${destination}`, 'snapshot_exists');
        }
        throw error;
      }
      return { valid: true, path: destination, compressedBytes: compressed.byteLength, manifest };
    } finally {
...
async function decodeEnvelope(compressed: Buffer): Promise<SnapshotEnvelope> {
  let raw: unknown;
  try {
    raw = JSON.parse((await gunzipAsync(compressed)).toString('utf8'));
  } catch (error) {
...
async function readBlobEntries(root: string): Promise<SnapshotBlobEntry[]> {
